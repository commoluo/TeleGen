...
def create_webvoyager_tasks_from_jsonl(jsonl_file, project_name, port=3000):
    """从jsonl文件创建WebVoyager测试任务"""
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # 为每个UI指令创建一个任务
        for j, ui_task in enumerate(ui_instructions):
            task = {
    ...
    # 保存任务文件
    task_file = f"./webvoyager_task_{project_name}_from_jsonl.jsonl"
...
def run_webvoyager_test_with_jsonl(jsonl_file, project_name, port=3000):
    """使用jsonl文件运行WebVoyager测试"""
    ...
    try:
        ...
        # 创建测试任务
        os.chdir(script_dir)
        task_file = create_webvoyager_tasks_from_jsonl(jsonl_file, project_name, port)
        ...
        # 运行WebVoyager测试
        print(f"\\n--- 步骤 4: 运行 WebVoyager 测试 ---")
        # 将输出目录和任务文件路径构造成绝对路径
        task_file_abs = Path(__file__).parent / task_file
        output_dir_abs = Path(results_dir) / f"{project_name}_jsonl_test"
...
def main():
    if len(sys.argv) < 2:
        print("使用方法: python test_with_jsonl.py <project_name> [port]")
        print("例如: python test_with_jsonl.py 001_simple_project_restructured 3000")
        print("\\n该脚本会使用../data/test.jsonl文件中的测试任务")
    ...
    success = run_webvoyager_test_with_jsonl(jsonl_file, project_name, port)
