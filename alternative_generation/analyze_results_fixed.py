...
def analyze_test_results(results_dir):
    ...
    for project in projects:
                ...
                try:
                    ...
                    if messages:
                        ...
                        if 'ANSWER' in content_upper or '任务完成' in assistant_content:
                            # 检查答案内容是否表示成功
                            negative_keywords = [
                                'NOT', 'NO', 'FAILED', 'ERROR', 'MISSING', 'INCORRECT', 
        ...
        print(f"  📈 项目成功率: {project_success}/{project_total} ({success_rate:.1f}%)")
    ...
    import re
# ... existing code ...
    print(f"总体成功率: {successful_tasks}/{total_tasks} ({overall_success_rate:.1f}%)")
    ...
    expected_projects = len(all_project_nums)
    ...
    for project in projects:
        match = re.search(r'_(\d+)_', project)
        if match:
            num = match.group(1)
            tested_project_nums.add(num.zfill(3)) # 保证三位数格式
...
def main():
    # 默认结果目录
    default_dir = Path(__file__).parent.parent / "webvoyager" / "webvoyager_results"
    
    if len(sys.argv) == 1:
        # 使用默认目录分析所有结果
        analyze_test_results(str(default_dir))
    elif len(sys.argv) == 2:
        # 指定目录或显示项目任务列表
        arg = sys.argv[1]
        if os.path.isdir(arg):
            ...
            # 如果不是目录，当作项目名显示任务列表
            show_task_details(str(default_dir), arg)
    elif len(sys.argv) == 3:
        # 显示指定任务的详细信息
        show_task_details(str(default_dir), sys.argv[1], sys.argv[2])
    else:
        print("使用方法:")
